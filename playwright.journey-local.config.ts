/*
 * Local Agent Configuration for Journey Tests
 *
 * This configuration runs tests directly on a local agent without Azure Playwright Workspaces.
 * Use playwright.journey.config.ts for Azure Workspace execution.
 *
 * Usage:
 *   npx playwright test --config=playwright.journey-local.config.ts
 */
...
console.log(`Playwright Journey Config (Local):`);
...
export default defineConfig({
    ...
    timeout: process.env.DEBUG_MODE ? 180000 : 900000,
    expect: {
    ...
    reporter: [
        ...
        [
            "@alex_neo/playwright-azure-reporter",
            {
                ...
                logging: true,
                testRunTitle: process.env.TEST_RUN_TITLE || `Journey Tests - ${AZURE_DEVOPS_DEFAULTS.ENVIRONMENT || "Local_Agent"} - ${new Date().toISOString().split("T")[0]}`,
                ...
                testCaseConfig: {
                    tags: "Journey;Automated;Playwright;Local_Agent",
                    priority: 1,
